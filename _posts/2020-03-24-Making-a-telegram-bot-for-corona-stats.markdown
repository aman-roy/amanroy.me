...
I used [worldinfo](https://www.worldometers.info/coronavirus/) website to get all the data. I noticed that all the data is kept here **inside the table**. So I fetched all the data inside the <tr> tag and from each <tr> tag, I fetched data from <td> tag. The function will take country name as an input and give back the stats related to it.
...
def get_data(country):
    ...
    table_data = [[cell.text.strip().strip(':').lower() \
    	for cell in row("td")] for row in BeautifulSoup(urlopen(url).read(), \
    		features="html.parser")("tr")]
